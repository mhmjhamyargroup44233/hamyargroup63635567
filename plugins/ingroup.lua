...
local function check_member_realmrem(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, '⛔️گروه حذف شد')
    end
  end
end
...
local function set_descriptionmod(msg, data, target, about)
  ...
  save_data(_config.moderation.data, data)
  return '✅درباره گروه تغییر یافت به:\n'..about
end
local function get_description(msg, data)
  local data_cat = 'description'
  if not data[tostring(msg.to.id)][data_cat] then
    return '⛔️توضیحی تنظیم نشده است'
  end
...
local function lock_group_chat(msg, data, target)
  ...
  if group_chat_lock == 'yes' then
    return '✔️چت قبلا قفل شده بود'
  else
    data[tostring(target)]['settings']['lock_chat'] = 'yes'
    save_data(_config.moderation.data, data)
    return '✅قفل چت فعال شد در ضورت فرستادن اولین پیام در گروه شخص اخراج خواهد شد'
  end
end
...
local function unlock_group_chat(msg, data, target)
  ...
  if group_chat_lock == 'no' then
    return '✔️قفل چت گروه قبلا باز شده بود میتوانید پیام دهید'
  else
    data[tostring(target)]['settings']['lock_chat'] = 'no'
    save_data(_config.moderation.data, data)
    return '⛔️قفل چت گروه غیر فعال شد میتوانید پیام دهید'
  end
end
...
local function lock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'yes' then
    return '✔️جلوگیری از ورود ربات به گروه قبلا فعال شده بود'
  else
    data[tostring(target)]['settings']['lock_bots'] = 'yes'
    save_data(_config.moderation.data, data)
    return '✅جلوگیری از ورود ربات به گروه فعال شد'
  end
end
...
local function unlock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'no' then
    return '✔️قفل ورود ربات قبلا غیر فعال شده بود'
  else
    data[tostring(target)]['settings']['lock_bots'] = 'no'
    save_data(_config.moderation.data, data)
    return '⛔️جلوگیری از ورود ربات غیر فعال شد'
  end
end
...
return '✔️قفل فحاشی قبلا فعال شده بود'
else
...
return '✔️قفل فحاشی قبلا غیر فعال شده بود'
else
data[tostring(target)]['settings']['antifosh'] = 'no'
save_data(_config.moderation.data, data)
return '⛔️قفل فحاشی غیر فعال شد'
end
end
...
return ' ✔️قفل لینک گروه قبلا غیر فعال شده بود '  
else
data[tostring(target)]['settings']['lock_join'] = 'yes'
save_data(_config.moderation.data, data)
return '✅قفل جلوگیری از ورود از طریق لینک فعال شد'
end
end
...
save_data(_config.moderation.data, data)
return '⛔️قفل لینک گروه غیر فعال شد'
end
end
...
return '✔️قفل تبلیغات قبلا قعال شده بود'
else
data[tostring(target)]['settings']['antiads'] = 'yes'
save_data(_config.moderation.data, data)
return '✅قفل جلوگیری از گذاشتن لینک تبلیغات فعال شد'
end
end
...
return '✔️قفل تبلیغات قبلا غیر فعال شده بود'
else
data[tostring(target)]['settings']['antiads'] = 'no'
save_data(_config.moderation.data, data)
return '⛔️قفل تبلیغات غیر فعال شد'
end
end
local function lock_group_namemod(msg, data, target)
  ...
  if group_name_lock == 'yes' then
    return '✔️قفل نام گروه قبلا فعال شده بود'
  else
    data[tostring(target)]['settings']['lock_name'] = 'yes'
    save_data(_config.moderation.data, data)
    rename_chat('chat#id'..target, group_name_set, ok_cb, false)
    return '✅قفل جلوگیری از تغییر نام گروه فعال شد'
  end
end
local function unlock_group_namemod(msg, data, target)
  ...
  if group_name_lock == 'no' then
    return '✔️قفل نام گروه قبلا غیر قعال شده بود'
  else
    data[tostring(target)]['settings']['lock_name'] = 'no'
    save_data(_config.moderation.data, data)
    return '⛔️قفل نام گروه غیر فعال شد'
  end
end
local function lock_group_floodmod(msg, data, target)
  ...
  if group_flood_lock == 'yes' then
    return '✅قفل جلوگیری از اسپم کردن در گروه فعال شد'
  else
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    return '✔️قفل عضوگیری قبلا فعال شده بود'
  else
    data[tostring(target)]['settings']['lock_member'] = 'yes'
    save_data(_config.moderation.data, data)
  end
  return '✔️قفل جلوگیری از عضو جدید و خصوصی کردن گروه فعال شد'
end
...
local function unlock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'no' then
    return '⛔️قفل عضو گیری غیر فعال شد'
  else
...
local function set_public_membermod(msg, data, target)
  ...
  return 'وضعیت گروه در حال حاضر:عمومی'
end
...
local function unset_public_membermod(msg, data, target)
  ...
  local group_member_lock = data[tostring(target)]['settings']['public']
  if group_member_lock == 'no' then
    return '✅گروه در حال حاضر خصوصی است'
  else
    data[tostring(target)]['settings']['public'] = 'no'
    save_data(_config.moderation.data, data)
    return 'وضعیت گروه در حال حاضر: خصوصی'
  end
end
...
local function lock_group_leave(msg, data, target)
  ...
  if leave_ban == 'yes' then
    return '✅قفل جلوگیری خارج شدن از گروه فعال شد'
  else
...
local function unlock_group_leave(msg, data, target)
  if not is_momod(msg) then
    return "⛔️شما مدیر نیستی"
  end
  local leave_ban = data[tostring(msg.to.id)]['settings']['leave_ban']
  if leave_ban == 'no' then
    return '⛔️قفل خارج شدن از گروه غیر فعال شد'
  else
...
local function unlock_group_photomod(msg, data, target)
  ...
  if group_photo_lock == 'no' then
    return '⛔️قفل عکس گروه غیر فعال شد'
  else
...
local function modadd(msg)
  ...
  if is_group(msg) then
    return '✔️گروه قبلا اضافه شده بود'
  end
    receiver = get_receiver(msg)
...
local function get_rules(msg, data)
  local data_cat = 'rules'
  if not data[tostring(msg.to.id)][data_cat] then
    return '⛔️قوانین تنظیم نشده است شما با این دستور setrules! میتوانید تنظیم کنید'
  end
...
local function set_group_photo(msg, success, result)
  ...
  if success then
    ...
    save_data(_config.moderation.data, data)
    send_large_msg(receiver, '✅عکس با موفقیت ثبت شد', ok_cb, false)
  else
...
local function promote(receiver, member_username, member_id)
  ...
  if not data[group] then
    return send_large_msg(receiver, '⛔️گروه اضافه نشد')
  end
  if data[group]['moderators'][tostring(member_id)] then
    return send_large_msg(receiver, member_username..' ✅به عنوان دستیار مدیر اضافه شد')
  end
...
local function demote(receiver, member_username, member_id)
  ...
  if not data[group]['moderators'][tostring(member_id)] then
    return send_large_msg(receiver, member_username..' ⛔️مقام دستیار مدیر گرفته شد')
  end
...
local function run(msg, matches)
  ...
  if msg.to.id and data[tostring(msg.to.id)] then
    ...
    if matches[1] == 'promote' and not matches[2] then
      if not is_owner(msg) then
        return "⛔️شما مدیر نیستید   فقط مالک گروه میتواند مقام دهد"
      end
    ...
    if matches[1] == 'link' then
      ...
      if not group_link then 
        return "✔️لطفا اول لینک را ایجاد کنید  برای اسجاد لینک کافی است از این دستور استفاده کنید newlink!"
      end
    ...
    if matches[1] == 'owner' then
      ...
      if not group_owner then 
        return "⛔️گروه مدیری ندارد   برای پیگیری میتوانید از پشتیبانی درخواست کنید @mhmafi "
      end
    ...
    if matches[1] == 'setflood' then 
      ...
      if tonumber(matches[2]) < 1 or tonumber(matches[2]) > 20 then
        return "⛔️شما فقط میتوانید از [1-20] اسپم را قفل کنید"
      end
      local flood_max = matches[2]
      data[tostring(msg.to.id)]['settings']['flood_msg_max'] = flood_max
      save_data(_config.moderation.data, data)
      savelog(msg.to.id, name_log.." ["..msg.from.id.."] set flood to ["..matches[2].."]")
      return '✅تعداد اسپم گروه تغییر یافت به  '..matches[2]
    end
    if matches[1] == 'clean' then
      ...
      if matches[2] == 'modlist' then
        if next(data[tostring(msg.to.id)]['moderators']) == nil then --fix way
          return '⛔️تعداد دستیار مدیر در گروه 0 است'
        end
        local message = '\nList of moderators for ' .. string.gsub(msg.to.print_name, '_', ' ') .. ':\n'
        for k,v in pairs(data[tostring(msg.to.id)]['moderators']) do
    ...
    if matches[1] == 'kickinactive' then
      --send_large_msg('chat#id'..msg.to.id, 'I\'m in matches[1]')
	    if not is_momod(msg) then
	      return '⛔️فقط مدیر میتواند افراد غیر فعال را حذف کند'
	    end
	    local num = 1
	    if matches[2] then
	        num = matches[2]
